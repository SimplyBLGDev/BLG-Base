...
If there's a related class that they depend on then make them belong to their 'namespace' (just a prefix) for example, if there's an Item label class as part of the HUD class which in turn is part of the UI it may be called UI_HUD_ItemLabel and it will be in the file UI_HUD_ItemLabel.gd

## VARIABLE NAMES
variables are in lower case and in case of multiple words are written_like_this, entirely in lowercase, even for words like hud and ui variables are always typed (unless there's a __really__ good reason not to), examples:
```gdscript
...
## GITIGNORE
The gitignore contains the LAUNCHSETTINGS.tres file, so as to avoid merge conflicts for simply different testing environments but this file needs to be uploaded to the initial repo first, make sure to manually commit it if it's not present in the repo
